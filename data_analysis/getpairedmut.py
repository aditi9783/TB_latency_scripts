...
# read H37Rv repeats and see if mutations lie in the repeat regions
invrepeatf = "/Users/aditigupta/Desktop/Roberto/LatencyProject/results_qscore200SNPs/H37Rv_repeats/einvert_outseq_H37Rv"
tandemrepeatf = "/Users/aditigupta/Desktop/Roberto/LatencyProject/results_qscore200SNPs/H37Rv_repeats/equicktandem_outfile_H37Rv"
invrepeats = readInvRepeats(invrepeatf) # tuples of start and end pos for each repeat
tandemrepeats = readTandemRepeats(tandemrepeatf)
print (set.intersection(set(synsnps), set(nonsynsnps)))

# check how many snps lie in repeat regions
snp_invrepeat = {} # key: snp, value: list depicting start end of repeat region
snp_tanrepeat = {}
for idx in indexid:
    for i in range(len(pairmutsnp[idx])):
        snp = pairmutsnp[idx][i]
        pos = int(snp[1:-1])
        invr = checkRegion(pos, invrepeats)
        tanr = checkRegion(pos, tandemrepeats)
        if len(invr) > 0: # repeat region returned, pos is in repeat regions
            snp_invrepeat[snp] = invr
        if len(tanr) > 0: # repeat region returned
            snp_tanrepeat[snp] = tanr

print ("Num snps in inv repeat regions:", len(snp_invrepeat))
print ("Num snps in tan repeat regions:", len(snp_tanrepeat))
print ("\ninv repeat region snps intersection intergenic:", set(snps_intergenic).intersection(set(snp_invrepeat)))
print ("tan repeat region snps intersection intergenic:", set(snps_intergenic).intersection(set(snp_tanrepeat)))

print ("\ninv repeat region snps intersection syn:", set(synsnps).intersection(set(snp_invrepeat)))
print ("tan repeat region snps intersection syn:", set(synsnps).intersection(set(snp_tanrepeat)))

print ("\ninv repeat region snps intersection nonsyn:", set(nonsynsnps).intersection(set(snp_invrepeat)))
print ("tan repeat region snps intersection nonsyn:", set(nonsynsnps).intersection(set(snp_tanrepeat)))

print ("\ninv repeat region snps intersection nonprot:", set(nonprotsnps).intersection(set(snp_invrepeat)))
print ("tan repeat region snps intersection nonprot:", set(nonprotsnps).intersection(set(snp_tanrepeat)))

# print repeat regions for the snps that are synonymous or non-synonymous
print("SNP in coding region that is in inv repeat (synonymous snp C2473699T):", snp_invrepeat['C2473699T'])
codinggene_snps_tanrepeats = ['T1441576C', 'C3603740A', 'A1441579T', 'T1441582C', 'A4053050G']
print("Coding region SNP that is in the tandem repeat:")
for cr in codinggene_snps_tanrepeats:
    print(cr, snp_tanrepeat[cr]) 

# when a list is saved as a set it gets rid of duplicate elements in the list. Save sets of mutgenes and then do an intersection to see which genes are mutated in all pairs
#setlist_mutgenes = []
#for idx in indexid:
#    setlist_mutgenes.append(set(pairmutgene[idx]))
#commonmutgenes = set.intersection(*setlist_mutgenes)
#print ("Genes mutated in all pairs:", commonmutgenes)

#setlist_snps = []
#for idx in indexid:
#    setlist_snps.append(set(pairmutsnp[idx]))
#commonsnps = set.intersection(*setlist_snps)
#print ("SNPs present in all pairs:", commonsnps)
